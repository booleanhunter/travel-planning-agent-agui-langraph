...
export function ElicitChipCard({ spec, onSubmit }: Props) {
    ...
    const fromMemoryBanner = useMemo(() => spec.fields.some((f) => f.prefilledFromMemory), [spec]);
                ...
                <div key={field.name} className="elicit-field">
                    <label>
                        ...
                        {field.prefilledFromMemory && (
                            <span className="from-memory">from memory</span>
                        )}
                    </label>
                    ...
                    {field.type === 'date-range' && (
                        <div style={{ display: 'flex', gap: 8 }}>
                            <input
                                ...
                                value={
                                    (values[field.name] as { start?: string } | undefined)?.start ??
                                    ''
                                }
                                onChange={(e) =>
                                    setValue(field.name, {
                            ...
                            />
                            <span style={{ alignSelf: 'center', color: 'var(--text-soft)' }}>
                                →
                            </span>
                            <input
                                type="date"
                                className="elicit-date"
                                value={
                                    (values[field.name] as { end?: string } | undefined)?.end ?? ''
                                }
                                onChange={(e) =>
                                    setValue(field.name, {
                                        ...(values[field.name] as object | undefined),
