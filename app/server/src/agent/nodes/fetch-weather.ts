...
export async function fetchWeather(state: AgentStateType): Promise<Partial<AgentStateType>> {
  ...
  console.log(`[fetch-weather] city=${state.destination} month=${weather.month} → ${weather.condition} (${weather.high}°/${weather.low}°C, ${Math.round(weather.precipitationChance * 100)}% rain)`);
  return { weather };
}
