...
| `03-langgraph-workflows.md` | Graph deep-dive | The three nodes, each with its own internal flow: `ContextRetriever` (parallel reads from Redis + Agent Memory (Iris)), `TravelAgent` (ReAct loop with five bound tools — `searchPois`, `getPoiDetails`, `getWeather`, `updateItinerary`, `saveTripToCalendar`), `FollowUp` (single LLM extraction call → buildElicit / suggestedActions → persist). |
...
| Redis-red `#ff4438` | `red` | Redis Stack (POI hybrid index, trip-store) |
| Blue `#80dbff` | `blue` | Frontend + backend services (Express, adapters) |
| Green `#7cc77f` | `green` | External services (Redis Agent Memory (Iris), OpenAI, OSM, Google APIs at runtime, Google Places at seed time) |
